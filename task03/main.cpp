    ...
    for (unsigned int jx = 0; jx < num_div; ++jx) {
      for (unsigned int jy = 0; jy < num_div; ++jy) {
        if (abs_diff(ix, jx) <= 1 && abs_diff(iy, jy) <= 1) { // this grid is near to the particle as the grid indexes are close
          ...
          unsigned int cnt = acc.grid2idx[jy * num_div + jx + 1] - acc.grid2idx[jy * num_div + jx];
          particles[ip].force += static_cast<float>(cnt) * gravitational_force(acc.grid2cg[jy * num_div + jx] - particles[ip].pos);
        }
      }
    }
  }
